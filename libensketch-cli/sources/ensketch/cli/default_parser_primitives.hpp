...
// using result = std::expected<type, error_type>;

// constexpr auto error(auto&& err) noexcept {
//   return unexpected(error_type{std::forward<decltype(err)>(err)});
// }
...
//                                            czstring str) -> result<type> {
//   return invoke(std::forward<decltype(self)>(self), meta::as_value<type>, str);
// };
