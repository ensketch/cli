...
![top-language-badge](https://img.shields.io/github/languages/top/ensketch/cli.svg?style=for-the-badge)
![code-size-badge](https://img.shields.io/github/languages/code-size/ensketch/cli.svg?style=for-the-badge)
![repo-size-badge](https://img.shields.io/github/repo-size/ensketch/cli.svg?style=for-the-badge)
[![license-badge](https://img.shields.io/github/license/ensketch/cli.svg?style=for-the-badge&color=blue)](#copyright-and-license)

![GitHub commit activity](https://img.shields.io/github/commit-activity/y/ensketch/cli?style=for-the-badge)
![GitHub last commit (by committer)](https://img.shields.io/github/last-commit/ensketch/cli?style=for-the-badge)
![GitHub tag (with filter)](https://img.shields.io/github/v/tag/ensketch/cli?style=for-the-badge)

[![cppget.org](https://img.shields.io/website/https/cppget.org/libensketch-cli.svg?down_message=offline&label=cppget.org&style=for-the-badge&up_color=blue&up_message=online)](https://cppget.org/libensketch-cli)

</div>

## Requirements and Dependencies

`cli` uses the [standard `build2` CI service][ci.cppget.org] to build and test its source code with a [variety of build configurations][ci.cppget.org-build-configs].
...
`cli` relies on [the `build2` toolchain][build2], and in order to use it, you should ensure that the toolchain is installed on your system.
...
:
role: prerequisite
location: https://github.com/ensketch/cli.git
```
...
```
import cli = libensketch-cli%lib{ensketch-cli}
exe{myexe}: {hxx cxx}{**} $cli
```
...
git clone --recurse https://github.com/ensketch/cli.git
cd cli
.develop/init.sh
```
...
`cli` does not include any publicly accessible configuration options.
...
`cli` is an open-source project, and we highly appreciate and value contributions from individuals like you.
...
If you have any questions or comments regarding `cli`, please don't hesitate to reach out to us at ensketch@mailbox.org.
...
Additionally, if you are incorporating `cli` into your projects, we'd greatly appreciate it if you could drop us a brief message.
...
`cli` is free software, distributed under the terms of the GNU General
Public License as published by the Free Software Foundation,
version 3 of the License (or any later version).  For more information,
see the [GNU General Public License][GPLv3] or the file [`COPYING.md`](COPYING.md).

`cli` is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more details.

Copyright years on `cli` source files may be listed using range notation, e.g., 1987-2012, indicating that every year in the range, inclusive, is a copyrightable year that could otherwise be listed individually.
...
[manifest]: libensketch-cli/manifest (libensketch-cli build2 Package Manifest)
[project-issues]: https://github.com/ensketch/cli/issues (Project Issues)
